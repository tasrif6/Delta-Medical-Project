...
export async function bookAppointment(formData) {
    ...
    try {
        ...
        //Check if the requisted time slot is available
        ...
        //Deduct credits from patients and add to doctor
...
//Video Token generate and credit deduction
...
export async function getDoctorById(doctorId) {
    try {
        ...
        console.log("Failed to fetch doctor", error.message)
    }
}
...
export async function getAvailableTimeSlots(doctorId) {
    try {
        ...
        for (const day of days) {
            ...
            availabilityStart.setFullYear( day.getFullYear(),
            ...
            while (isBefore(addMinutes(current, 30), end) || +addMinutes(current, 30) === +end) {
                ...
                const overlaps = existingAppointments.some((appointment) => {
                    ...
                    return (
                        (current >= aStart && current < aEnd) || (next > aStart && next <= aEnd) || (current <= aStart && next >= aEnd)
                    );
                });
                ...
                if (!overlaps) {
                    availableSlotsByDay[dayString].push({
                        ...
                        formatted: `${format(current, "h:mm a")} -${format(next, "h:mm a")}`, day: format(current, "EEE, MMM d"),
                    });
                }
    ...
    const result = Object.entries(availableSlotsByDay).map(([date, slots]) => ({
        date, displayDate: slots.length > 0 ? slots[0].day : format(new Date(date), "EEE, MMM d"), slots, 
    }));
